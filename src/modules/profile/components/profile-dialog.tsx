...
export const ProfileDialog = ({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) => {
  const { t, language } = useLanguage() as { t: any, language: string };
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={(val) => { if (!val) handleAttemptClose(); }}>
        ...
        >
          <DialogHeader className="border-b pb-4 text-left">
            <DialogTitle className="text-xl font-bold text-slate-800">{t.updateProfileTitle}</DialogTitle>
          </DialogHeader>
          ...
          {isFetching ? (
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleOnSubmit)} className="space-y-5 pt-4">
                ...
                <DialogFooter className="pt-6 border-t gap-2 flex justify-end">
                  <Button type="button" variant="ghost" onClick={handleAttemptClose} className="px-6 font-bold text-slate-500 hover:bg-slate-100">
                    {t.cancel}
                  </Button>
                  <Button 
                    type="submit" 
                    disabled={updateMutation.isPending}
                    className="px-10 font-bold bg-pink-500 hover:bg-pink-600 text-white rounded-xl shadow-md transition-all active:scale-95"
                  >
      ...
      {showExitConfirm && (
        <AlertDialog key={language} open={true} onOpenChange={setShowExitConfirm}>
          <AlertDialogContent className="max-w-[400px] rounded-xl p-0 overflow-hidden border-none shadow-2xl"> 
            ...
            <div className="p-6">
              <AlertDialogHeader className="text-left">
                ...
                <AlertDialogDescription className="text-slate-500 text-sm mt-3 leading-relaxed">
                  {t.leavePageDescription}
              ...
              <div className="flex justify-end gap-2 pt-8">
                ...
                <AlertDialogAction 
                  ...
                  className="h-10 px-8 rounded-lg font-bold bg-red-600 hover:bg-red-700 text-white text-sm shadow-md transition-all active:scale-95"
                >
                  {t.ok}
                </AlertDialogAction>
              </div>
