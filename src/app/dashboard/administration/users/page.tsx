...
export default function UsersPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const [resList, resCount] = await Promise.all([
        userApi.getUsers({ orgId, offset: page * rowsPerPage, limit: rowsPerPage, fullTextSearch: searchTerm }),
        userApi.getUserCount({ orgId, fullTextSearch: searchTerm })
      ]);
  ...
  const confirmStatusChange = async () => {
    ...
    try {
      ...
      toast.success(`Successfully ${statusTarget.action}d user`);
      fetchUsers();
    } catch (error) { toast.error(`Failed to change status`); }
    finally { setIsStatusModalOpen(false); setStatusTarget(null); }
  };
  ...
  const confirmBulkDelete = async () => {
    ...
    finally { setIsDeleteModalOpen(false); setSelectedIds(new Set()); }
  };
    ...
    <div className="flex-1 flex flex-col space-y-4 overflow-hidden min-h-0"> 
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center shrink-0">
        <div className="flex items-center gap-2 w-full md:w-auto flex-1 max-w-2xl">
            ...
            <button onClick={fetchUsers} className="h-[40px] w-[40px] bg-pink-400 hover:bg-pink-500 text-white rounded-lg flex items-center justify-center shadow-pink-100 transition-all active:scale-95">
                <Search size={18} />
            </button>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
            ...
            <button 
              ...
              className={`flex items-center justify-center px-6 py-2 text-sm font-bold rounded-lg shadow-md transition-all active:scale-95 ${
                ...
                : "bg-red-300 text-white cursor-not-allowed shadow-none" // สีแดงอ่อนตอน Disable
              }`}
            >
        ...
        <div className="flex-1 overflow-auto no-scrollbar"> 
            <table className="w-full text-left border-collapse min-w-[1100px]">
                <thead className="sticky top-0 bg-slate-50/90 backdrop-blur-md z-40 border-b border-slate-100">
                    <tr>
                        <th className="px-4 py-3 w-[50px] bg-slate-50"><input type="checkbox" checked={users.length > 0 && selectedIds.size === users.length} onChange={toggleSelectAll} className="w-4 h-4 rounded border-slate-300 text-pink-400 cursor-pointer" /></th>
                        <th className="px-4 py-3 text-sm font-bold text-slate-800">Username</th>
                        <th className="px-4 py-3 text-sm font-bold text-slate-800">Email</th>
                        <th className="px-4 py-3 text-sm font-bold text-slate-800">Tags</th>
                        <th className="px-4 py-3 text-sm font-bold text-slate-800">Custom Role</th>
                        <th className="px-4 py-3 text-sm font-bold text-slate-800">Role</th>
                        <th className="px-4 py-3 text-sm font-bold text-slate-800 text-center min-w-[120px]">Initial User</th>
                        <th className="px-4 py-3 text-sm font-bold text-slate-800 sticky right-[85px] bg-slate-50/90 z-40 text-center min-w-[100px]">Status</th>
                        <th className="px-4 py-3 text-sm font-bold text-slate-800 text-center sticky right-0 bg-slate-50/90 z-40 shadow-[-4px_0_10px_-4px_rgba(0,0,0,0.05)] min-w-[90px]">Action</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {isLoading ? (
                    ...
                    users.map((user, index) => {
                        ...
                        const isInitial = user.isOrgInitialUser === "true" || user.isOrgInitialUser === "1";
                        const isMenuOpen = openMenuId === uid;

                        return (
                        <tr key={uid} className={`transition-all group ${isSelected ? "bg-pink-50/50" : "hover:bg-slate-50/50"}`}>
                            <td className="px-4 py-3"><input type="checkbox" checked={isSelected} onChange={() => toggleSelectOne(uid)} className="w-4 h-4 rounded border-slate-300 text-pink-400 cursor-pointer" /></td>
                            
                            <td className="px-4 py-3 text-sm font-medium text-pink-600 whitespace-nowrap hover:underline cursor-pointer">
                                {user.userName}
                            </td>
                            ...
                            <td className={`px-4 py-3 text-center sticky right-[85px] z-30 transition-colors ${isSelected ? "bg-[#fff5f6]" : "bg-white group-hover:bg-slate-50"}`}>
                                <span className="text-[13.5px] font-medium text-slate-900 capitalize">
                                    {user.userStatus || "Active"}
                                </span>
                            </td>
                            
                            <td className={`px-4 py-3 text-center sticky right-0 transition-colors shadow-[-4px_0_10px_-4px_rgba(0,0,0,0.1)] ${isMenuOpen ? "z-50" : "z-30"} ${isSelected ? "bg-[#fff5f6]" : "bg-white group-hover:bg-slate-50"}`}>
                                <div onClick={(e) => e.stopPropagation()} className="relative">
                                    ...
                                    {isMenuOpen && (
                                        <div className="absolute right-8 top-full mt-1 w-44 bg-white rounded-xl shadow-2xl border border-slate-100 z-[60] py-1.5 text-left animate-in fade-in zoom-in-95 duration-150 origin-top-right">
                                            <button disabled={isStatusDisabled} onClick={() => handleStatusClick(uid, user.userStatus)} className={`flex items-center gap-2 px-4 py-2 text-[13.5px] font-medium w-full transition-colors ${isStatusDisabled ? "text-slate-300 cursor-not-allowed" : "text-slate-600 hover:bg-slate-50"}`}>Disable User</button>
                                            <button disabled={!isStatusDisabled} onClick={() => handleStatusClick(uid, user.userStatus)} className={`flex items-center gap-2 px-4 py-2 text-[13.5px] font-medium w-full transition-colors ${!isStatusDisabled ? "text-slate-300 cursor-not-allowed" : "text-slate-600 hover:bg-slate-50"}`}>Enable User</button>
                                        </div>
                                    )}
                                </div>
                            </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-150">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                    <h3 className="text-lg font-black text-slate-800">{statusTarget?.action} User</h3>
                ...
                <div className="px-6 py-8 text-slate-600 text-base leading-relaxed">
                    Are you sure you want to <span className="font-black text-slate-900">{statusTarget?.action.toLowerCase()}</span> this User?
                </div>
                <div className="flex items-center justify-end gap-3 px-6 py-4 bg-slate-50 border-t border-slate-100">
                    <button onClick={() => setIsStatusModalOpen(false)} className="px-6 py-2 text-sm font-bold text-slate-700 bg-[#f1f1f1] rounded-xl hover:bg-slate-200 transition-colors">Cancel</button>
                    <button 
                        onClick={confirmStatusChange} 
                        className={`px-10 py-2 text-sm font-black text-white rounded-xl shadow-lg transition-all active:scale-95 ${
                            statusTarget?.action === 'Disable' ? 'bg-red-600 hover:bg-red-700 shadow-red-100' : 'bg-pink-500 hover:bg-pink-600 shadow-pink-100'
                        }`}
                    >
                        OK
                    </button>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95">
                ...
                <div className="px-6 py-8 text-slate-600 text-base leading-relaxed">Are you sure you want to delete the selected <span className="font-black text-red-600">{selectedIds.size}</span> user(s)?</div>
                <div className="flex items-center justify-end gap-3 px-6 py-4 bg-slate-50 border-t border-slate-100">
                    <button onClick={() => setIsDeleteModalOpen(false)} className="px-6 py-2 text-sm font-bold text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 shadow-sm transition-colors">Cancel</button>
