...
export default function AuditLogPage() {
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-140px)] flex flex-col"> 
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 items-center shrink-0">
        ...
        <div className="flex items-center gap-2 w-full flex-1">
            ...
            {/* Date Range Picker */}
            <div className="hidden md:flex items-center gap-2 px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-600 min-w-[240px] cursor-pointer hover:bg-slate-50 transition-colors">
                <Calendar size={16} className="text-slate-400" />
                <span>{t.dateRangePlaceholder}</span>
            </div>

            {/* Search Button */}
            <button className="h-[42px] w-[42px] bg-blue-500 hover:bg-blue-600 text-white rounded-lg flex items-center justify-center transition-colors shadow-sm shadow-blue-200">
                <Search size={20} />
            </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm flex-1 flex flex-col overflow-hidden relative">
            ...
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/50">
                        <th className="p-4 text-sm font-semibold text-slate-800">{t.time}</th>
                        <th className="p-4 text-sm font-semibold text-slate-800">{t.username}</th>
                        <th className="p-4 text-sm font-semibold text-slate-800">{t.idType}</th>
                        <th className="p-4 text-sm font-semibold text-slate-800">{t.api}</th>
                        <th className="p-4 text-sm font-semibold text-slate-800">{t.status}</th>
                        <th className="p-4 text-sm font-semibold text-slate-800">{t.role}</th>
                        <th className="p-4 text-sm font-semibold text-slate-800">{t.ipAddress}</th>
        ...
        <div className="p-4 border-t border-slate-100 flex items-center justify-end gap-6 text-sm text-slate-500 bg-white absolute bottom-0 w-full">
            ...
            <div className="flex items-center gap-1">
                <button className="p-1.5 rounded-full hover:bg-slate-100 text-slate-500 transition-colors active:bg-slate-200">
                    <ChevronLeft size={20} />
                </button>
                <button className="p-1.5 rounded-full hover:bg-slate-100 text-slate-500 transition-colors active:bg-slate-200">
